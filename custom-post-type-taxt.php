...
function dave_register_post_type($name, $singular, $multiple, $slug, $icon, $args = []) {
  $defaults = [
    'labels' => [
      'name' => $multiple,
...
function dave_register_taxonomy($name, $singular, $multiple, $slug, $postTypes = [], $args = []) {
  $defaults = [
    'labels' => [
      'name' => $multiple,
      'singular_name' => $singular,
...
function dave_register_cpt_and_tax() {
  dave_register_post_type('name-cpt', 'label-cpt', 'plural-cpt', 'slug-cpt', 'name-icon');
  dave_register_taxonomy('category-cpt', 'Category label', 'Categories plural', 'category-slug', ['name-cpt']);


  // COMPILE_CODE_HERE: aggiungere registrazione di eventuali custom post type
}
add_action( 'init', 'dave_register_cpt_and_tax');
